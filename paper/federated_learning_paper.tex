...
\begin{figure}[t]
    ...
    \caption{Convergence curves comparing aggregation algorithms. TrimmedMean 160r achieves highest final accuracy. Krum fails to converge. FedAvg converges quickly but remains vulnerable to attacks.}
...
ATMA achieves 65.78\% accuracy on CIFAR-10 (160 rounds) with Dirichlet($\alpha$=0.5) non-IID distribution, competitive with TrimmedMean's 67.92\% (see Table~\ref{tab:cifar10_results}). This validates ATMA's core advantage: \emph{adaptive defense without manual threshold tuning}.
