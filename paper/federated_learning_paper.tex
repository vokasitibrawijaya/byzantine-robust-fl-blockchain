...
\author{\uppercase{[Author Names Withheld for Blind Review]}\authorrefmark{1},
\IEEEmembership{Member, IEEE}}

\address[1]{[Affiliation Withheld for Blind Review]}
\address[2]{[Affiliation Withheld for Blind Review]}

\tfootnote{This work was supported in part by Research Grant.}

\markboth
{Author: Byzantine-Robust Federated Learning with Adaptive Aggregation and Blockchain}
{Author: Byzantine-Robust Federated Learning with Adaptive Aggregation and Blockchain}

\corresp{Corresponding author: [Email withheld for blind review].}
...
XII (CIFAR-10) & CIFAR-10 & ResNet-18 & Label-flip ($\lambda$=-5) & 4/20 (20\%) & 50-160 & 42 & 5 & cifar10\_simple.json \\
XIII (Multi-seed) & CIFAR-10 & ResNet-18 & Label-flip ($\lambda$=-5) & 4/20 (20\%) & 50 & 42-44$^*$ & 3$^\dagger$ & multiseed.json \\
XIV (Gas Cost) & N/A & N/A & N/A & N/A & N/A & N/A & N/A & blockchain\_cost.json \\
\bottomrule
\end{tabular}
\\[0.1cm]
\small $^*$Multi-seed CI uses seeds 42, 43, 44. $^\dagger$Table XIII uses reduced local\_epochs=3, batch=256 for rapid evaluation (explains 34.62\% vs 66.38\% in Table XII).
\end{table*}
...
The centralized system achieves AUC=0.000 \emph{by design}: without immutable logs, all provenance queries return ``unverifiable,'' producing identical true/false positive rates (i.e., random classification on the tamper-detection task). This confirms that blockchain immutability is \emph{necessary} for reliable provenance verification---it is not a bug but demonstrates that mutable logging systems cannot distinguish legitimate from tampered records.
