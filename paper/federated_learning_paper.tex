...
\tfootnote{This work was supported by the Laboratory of Internet of Things \& Human Centered Design, Faculty of Vocational Studies, Universitas Brawijaya.}
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Aggregation Validation:} We validate state-of-the-art ATMA~\cite{kalibbala2025atma} for non-IID data, demonstrating 85.12\% accuracy with dynamic threshold adaptation (0.15-0.24) that shows +0.73\% improvement over static TrimmedMean in our blockchain environment tests.
...
\textbf{FedCmp}~\cite{li2024fedcmp} presents comprehensive experimental comparisons of Byzantine defenses, providing valuable benchmarks. Our work extends this by integrating blockchain auditability.
...
\begin{itemize}
    \item \textbf{Code Repository:} Complete implementation available at:
    
    {\small\path{https://github.com/vokasitibrawijaya/byzantine-robust-fl-blockchain}}
...
These results provide strong evidence that Byzantine-robust aggregation does not require sacrificing model performance. In fact, TrimmedMean's statistical robustness may help filter noise and outliers, leading to better convergence.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\textwidth]{convergence_comparison.png}
...
\begin{figure}[t]
    ...
    \caption{H2 Provenance Detection ROC Curve. Blockchain system achieves strong discriminative power (AUC=0.957), detecting 81\% of attacks with zero false positives. Centralized system with mutable logs shows zero detection capability (AUC=0.0) as attackers can tamper logs. Results from valid rerun experiment (seed=42, 50 rounds, 30\% attack probability).}
    \label{fig:h2_roc}
\end{figure}
...
The centralized system achieves AUC=0.000 \emph{by design}: this comparison tests \textbf{log immutability}, not detection algorithm quality. In centralized systems without immutable logs, sophisticated attackers can tamper with detection records post-hoc, making all provenance queries return ``unverifiable.'' This produces identical true/false positive rates (random classification on the tamper-detection task). 

\textbf{Clarification:} We are \emph{not} comparing blockchain detection vs. centralized anomaly detection algorithms (e.g., Mahalanobis distance, Isolation Forest). Rather, we demonstrate that blockchain immutability is \emph{necessary} for reliable provenance verification---without it, even perfect detection is meaningless since records can be altered. Future work could compare blockchain+detection vs. centralized+detection for algorithm-level analysis.
...
\begin{figure}[t]
    ...
    \caption{Real FL Training on MNIST (20 rounds). Left: Test accuracy progression showing clear learning trajectory. TrimmedMean achieves highest final accuracy (73\%), followed by Median (71.48\%) and Mean (66.95\%). Right: Final accuracy comparison with 95\% confidence intervals.}
...
\textbf{Extended Training Reference:} While our ATMA experiments use 50 rounds for controlled comparison, we also evaluate ATMA under extended training in the CIFAR-10 experiments (Section~\ref{sec:cifar10}). At 160 rounds, ATMA achieves \textbf{65.78\% accuracy} on CIFAR-10 with Dirichlet($\alpha$=0.5) non-IID distribution, competitive with TrimmedMean's 67.92\%. This demonstrates that ATMA's adaptive benefits persist across training durations, though static TrimmedMean's simpler approach achieves slightly higher peak accuracy with sufficient training budget.
...
Table~\ref{tab:literature_comparison} provides context for our results relative to literature. Direct comparison is challenging due to differences in Byzantine attack configurations, non-IID distributions, and training settings.

\begin{table}[t]
\caption{Comparison with Literature (Reference Context Only$^*$)}
...
\\[0.1cm]
{\footnotesize $^*$Not directly comparable due to different attack configurations and training settings. Provided for general context only.}
\end{table}

Our TrimmedMean 160r result (93.45\%) is competitive with state-of-the-art Byzantine-robust methods on MNIST. Direct comparison with prior work is limited by differences in threat model (attack type, Byzantine fraction), non-IID distribution, and communication budget. We report this result specifically for our setting: 20\% label-flip attackers, IID distribution, 160 communication rounds with blockchain integration.
...
\textbf{L2 Cost Projections:} The 50--100x cost reduction for Layer-2 networks (Arbitrum, Optimism) is based on published benchmarks~\cite{arbitrum2023} and Arbitrum documentation, not measured deployments.

\textbf{Testnet Deployment Plan:} We have prepared complete deployment infrastructure for Sepolia (L1) and Arbitrum Sepolia (L2) testnets, including smart contracts, deployment scripts, and wallet (\texttt{0x66Cd...8d17}). Deployment requires 0.5 ETH testnet funding which was not obtained due to faucet availability constraints. All deployment code is available in the repository for future validation.
...
\begin{enumerate}
    ...
    \item \textbf{Confidence Intervals:} \textbf{ADDRESSED} --- Multi-seed experiments (42, 43, 44) provide statistical rigor.
    ...
    \begin{itemize}
        \item Attack types limited to random noise Byzantine attacks; sophisticated attacks (label-flipping with ALIE, backdoor~\cite{sun2019backdoor,nguyen2022flame}) require future study.
        \item Scalability validated to 1,000 clients; 10,000+ requires additional testing.
        \item \textbf{Blockchain Environment:} Ganache local simulation provides accurate gas measurements but does not capture real-world network conditions (12s block time latency, mempool congestion, gas price volatility 10--500+ Gwei). Testnet deployment infrastructure is prepared (wallet: \texttt{0x66Cd...8d17}, contracts ready) but requires 0.5 ETH testnet funding. Production deployment validation on Sepolia/Arbitrum testnets is recommended future work.
        \item \textbf{L2 Cost Projections:} The 50--100x Layer-2 cost reduction is based on Arbitrum documentation and published benchmarks, not measured deployments.
    \end{itemize}
\end{enumerate}
