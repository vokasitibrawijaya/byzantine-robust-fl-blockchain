...
\textbf{``Byzantine-Robust Federated Learning with Adaptive Aggregation and Blockchain: Empirical Validation of ATMA and Resolution of the Transparency Paradox''}\\
2026-04-13

We sincerely thank the editor and reviewers for their constructive feedback. We have carefully addressed every comment below. The revised manuscript follows this response, and the major changes are described below. Page and section numbers refer to the revised manuscript that follows this response.

\subsection*{Reviewer A}
\textbf{Comment A1:} ``Attack types are limited to label-flip and random noise; sophisticated attacks (ALIE [18], backdoor [20]) require future study.'' Rephrase this as the citations look strange now.

\textbf{Answer:} We agree that the inline citation format was unclear. We have rephrased the sentence in the limitations section as follows: ``Attack types are limited to random noise and label-flip attacks. More sophisticated attack strategies, such as the `A Little Is Enough' (ALIE) approach proposed by Baruch et al. [10] and backdoor attacks studied by Sun et al. [11], were not fully evaluated in this work and require future study.'' The citation numbers now clearly refer to the named authors and their specific attack methods.

\textbf{Comment A2:} Figures 1--2 not readable.

\textbf{Answer:} We regenerated both figures at publication quality. Figure 1 now uses 300 DPI resolution, distinct markers, thicker lines, larger labels, and a clearer legend. Figure 2 now uses 300 DPI resolution, thicker ROC curves, annotated operating point, larger labels, and clear AUC values.

\textbf{Comment A3:} Conclusion (and article) too short I think.

\textbf{Answer:} We substantially expanded the conclusion. The revised conclusion now includes: (1) summary of key findings with accuracy values and statistical tests, (2) comparative analysis with prior work, (3) practical contributions and deployment guidelines, (4) limitations, and (5) future work.

\textbf{Comment A4:} Expand it to include comparative analysis, contribution discussion.

\textbf{Answer:} Addressed in Comment A3. The revised conclusion includes a structured comparative analysis against key prior studies and an explicit discussion of practical contributions for Byzantine-robust federated learning deployment.

\subsection*{Reviewer B}
\textbf{Comment B1:} It is not clear what EXACTLY the authors did and how (step by step).

\textbf{Answer:} We added a new step-by-step experimental procedure describing exactly what was executed: data loading with \texttt{torchvision.datasets.MNIST} and \texttt{torchvision.datasets.CIFAR10}; normalization using dataset-specific $\mu$ and $\sigma$ values; Dirichlet($\alpha=0.5$) non-IID partitioning using \texttt{numpy.random.dirichlet}; model initialization; Byzantine client selection; Ganache/Solidity blockchain deployment; local SGD training; label-flip and scaling attack injection with $\lambda=-5.0$; SHA-256 blockchain submission; server-side aggregation; Byzantine detection logging; global update; testing; JSON serialization; and multi-seed replication.

\textbf{Comment B2:} There is no point in throwing a lot of terms without being specific on what you did exactly and how. Not generic information. SPECIFICALLY in this work.

\textbf{Answer:} We revised the paper to use specific implementation details instead of generic descriptions. The revised manuscript specifies exact algorithms, parameter values, datasets, seeds, smart-contract functions, gas costs, result files, and reproducibility details.

\begin{center}\textbf{REVISED MANUSCRIPT FOLLOWS}\end{center}


\begin{center}
{\LARGE\textbf{Byzantine-Robust Federated Learning with Adaptive Aggregation and Blockchain: Empirical Validation of ATMA and Resolution of the Transparency Paradox}}\\[12pt]
Rachmad Andri Atmoko$^{1,2}$, Sholeh Hadi Pramono$^{1}$, M. Fauzan Edy Purnomo$^{1}$, Panca Mudjirahardjo$^{1}$, Mahdin Rohmatillah$^{1}$, and Cries Avian$^{1}$\\[10pt]
...
\textit{Corresponding author: Sholeh Hadi Pramono (e-mail: sholehpramono@ub.ac.id)}\\[8pt]
January 2026
\end{center}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Attack types limited to random noise and label-flip attacks; sophisticated attacks (ALIE~[10], backdoor~[11]) require future study.
