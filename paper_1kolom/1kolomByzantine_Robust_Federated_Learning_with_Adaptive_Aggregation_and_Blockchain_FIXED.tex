...
\title{\textbf{Byzantine-Robust Federated Learning with Adaptive Aggregation and Blockchain: Empirical Validation of ATMA and Resolution of the Transparency Paradox}}

\author{
Rachmad Andri Atmoko$^{1,2}$,
Sholeh Hadi Pramono$^{1}$,
M. Fauzan Edy Purnomo$^{1}$,
Panca Mudjirahardjo$^{1}$,
Mahdin Rohmatillah$^{1}$, and
Cries Avian$^{1}$\\[10pt]
$^{1}$Electrical Engineering Department, Faculty of Engineering, Universitas Brawijaya, Malang 65145, Indonesia\\
$^{2}$Faculty of Vocational Studies, Universitas Brawijaya, Malang 65145, Indonesia\\[5pt]
\textit{Corresponding author: Sholeh Hadi Pramono (e-mail: sholehpramono@ub.ac.id)}
}
...
\noindent\textbf{Keywords:} Byzantine-robust aggregation, blockchain, federated learning, TrimmedMean, ATMA, adaptive attacks, transparency paradox, model poisoning, distributed machine learning, privacy-preserving learning
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Aggregation Validation:} We validate ATMA~[46] for non-IID data, achieving 85.12\% accuracy with dynamic threshold adaptation (0.15-0.24) and +0.73\% improvement over static TrimmedMean in blockchain environment tests.
...
\textbf{Krum} [3] selects the most representative model update based on geometric proximity to other updates. While theoretically sound, Krum's conservative selection can reject legitimate updates, potentially hindering convergence.
...
\textbf{TrimmedMean and Median} [4] compute coordinate-wise statistics after removing extreme values. These methods have shown strong Byzantine resilience in distributed optimization.

\textbf{Bulyan} [5] combines Krum selection with coordinate-wise median computation for enhanced security.
...
\textbf{FedSV}~[36] introduces Shapley value-based contribution assessment, enabling fair client valuation under Byzantine attacks. Unlike our trimming approach, FedSV requires additional computation for game-theoretic valuation.

\textbf{LASA}~[37] proposes layer-adaptive sparse aggregation, achieving robustness through sparsification rather than statistical trimming. This complements our approach by offering alternative defense mechanisms.

\textbf{FedCmp}~[38] presents experimental comparisons of Byzantine defenses, providing valuable benchmarks. Our work extends this by integrating blockchain auditability.

\textbf{2024 Survey}~[39] provides updated taxonomy of Byzantine attacks and defenses, categorizing methods by detection vs. tolerance approaches. Our ATMA evaluation follows their recommended adaptive defense category.
...
We consider a threat model where a fraction $\alpha$ of clients are Byzantine [2], meaning they can send arbitrary model updates to disrupt training. Let $\mathcal{B} \subset \{1, \ldots, N\}$ denote the set of Byzantine clients with $|\mathcal{B}| = \lfloor \alpha N \rfloor$. Byzantine clients can:
...
\label{sec:step_by_step}

To ensure clarity and reproducibility, we describe the exact procedure executed in each experiment:

\begin{enumerate}
    \item \textbf{Data Loading and Preprocessing.} We download MNIST (via \texttt{torchvision.datasets.MNIST}) and CIFAR-10 (via \texttt{torchvision.datasets.CIFAR10}). MNIST images are normalized with $\mu=0.1307$, $\sigma=0.3081$. CIFAR-10 images are normalized per-channel ($\mu=[0.4914, 0.4822, 0.4465]$, $\sigma=[0.2470, 0.2435, 0.2616]$).
    
    \item \textbf{Non-IID Data Partitioning.} For each of the 10 classes, we sample client proportions from a Dirichlet distribution with $\alpha=0.5$. Each client $i$ receives data indices proportional to these drawn proportions, resulting in heterogeneous label distributions across clients. For example, one client may receive 60\% images of digit ``3'' and few images of digit ``7'', while another client has the opposite distribution. This is implemented using \texttt{numpy.random.dirichlet}.
    
    \item \textbf{Model Initialization.} For MNIST, we use SimpleMNISTNet: two convolutional layers (16 and 32 filters, $3 \times 3$ kernels, ReLU, $2 \times 2$ max-pool) followed by two fully connected layers (128 and 10 units). For CIFAR-10, we use ResNet-18 adapted for $32 \times 32$ input. All model weights are initialized identically on all clients using the same random seed (42).
    
    \item \textbf{Byzantine Client Selection.} We randomly select 4 out of 20 clients as Byzantine (20\% ratio). Byzantine client IDs are fixed throughout training for each seed.
    
    \item \textbf{Blockchain Contract Deployment.} Before training begins, we deploy our \texttt{FederatedLearningAggregator} Solidity smart contract to Ganache (local Ethereum network) using \texttt{solcx} compiler (Solidity~0.8.19). The contract address, ABI, and deployment gas cost (1,724,238 gas) are recorded.
    
    \item \textbf{Training Loop (repeated for $T=50$ or $T=160$ rounds):}
    \begin{enumerate}
        \item \textbf{Client Selection:} Randomly select 10 of 20 clients (50\% participation) for this round.
        \item \textbf{Local Training:} Each selected client receives a copy of the current global model, then trains locally using SGD (learning rate 0.01 for MNIST, 0.01 for CIFAR-10) for 5 local epochs on its private data partition (batch size 32). The client computes its model update as $\Delta \mathbf{w}_i = \mathbf{w}_{\text{local}} - \mathbf{w}_{\text{global}}$.
        \item \textbf{Byzantine Attack Injection:} Each Byzantine client (if selected this round) modifies its update: (a)~flips training labels $y' = (y+1) \bmod 10$ during local training, and (b)~scales the resulting gradient by $\lambda = -5.0$, producing $\Delta \mathbf{w}_{\text{byz}} = -5.0 \cdot \Delta \mathbf{w}_i$. Byzantine clients activate with 80\% probability per round.
        \item \textbf{Blockchain Submission:} Each client submits a SHA-256 hash of its model update to the smart contract via \texttt{submitUpdate(clientId, modelHash, round)}. Gas usage per submission ($\approx$75,000 gas) is recorded.
        \item \textbf{Server-Side Aggregation:} The server applies the selected aggregation algorithm:
        \begin{itemize}
            \item \emph{FedAvg:} Weighted average of all received updates.
            \item \emph{Krum:} Select the single update with the smallest sum of squared distances to its $n - f - 2$ nearest neighbors.
            \item \emph{TrimmedMean:} For each model parameter coordinate, sort the $n$ client values, remove the top and bottom 20\% ($\beta = 0.2$), and average the remaining 60\%.
            \item \emph{ATMA:} Compute coordinate-wise median and Median Absolute Deviation (MAD). Retain only updates within $\tau_t \times \text{MAD}$ of the median, where $\tau_t$ adapts each round based on gradient variance.
        \end{itemize}
        \item \textbf{Byzantine Detection Logging:} If the aggregation algorithm identifies outlier updates (e.g., updates trimmed by TrimmedMean or rejected by ATMA), the server calls \texttt{markByzantine(clientId, round)} on the smart contract, creating an immutable on-chain record.
        \item \textbf{Global Model Update:} The server updates the global model: $\mathbf{w}_{t+1} = \mathbf{w}_t + \text{Aggregate}(\{\Delta \mathbf{w}_i\})$.
        \item \textbf{Round Completion:} The server calls \texttt{completeRound(aggregatedHash)} on-chain, storing the SHA-256 hash of the aggregated model.
        \item \textbf{Evaluation:} The updated global model is evaluated on the full test set (10,000 images). Test accuracy and cross-entropy loss are recorded.
    \end{enumerate}
    
    \item \textbf{Result Serialization.} All per-round metrics (accuracy, loss, gas costs, detected Byzantine clients, timestamps) are saved to JSON files for reproducibility and post-hoc analysis.
    
    \item \textbf{Multi-Seed Replication.} Steps 1--7 are repeated with seeds 42, 43, and 44 to compute mean accuracy, standard deviation, and 95\% confidence intervals $(\bar{x} \pm t_{0.025,2} \cdot s / \sqrt{3})$.
\end{enumerate}

All code implementing the above procedure is publicly available at \url{https://github.com/vokasitibrawijaya/byzantine-robust-fl-blockchain}.
...
\textbf{Attack Severity Context:} Our aggressive attack configuration (label-flip with scale=-5.0) creates extreme gradient perturbations that exploit Krum's core weakness: single-update selection. In literature, Krum typically achieves 70--80\% on MNIST without attacks~[3], suggesting our attack causes severe degradation.
...
\begin{figure}[t]
    ...
    \caption{Convergence curves comparing aggregation algorithms on CIFAR-10 with Dirichlet($\alpha$=0.5) non-IID distribution under 20\% Byzantine attack (label-flip, scale=-5.0). TrimmedMean achieves the highest final accuracy (67.92\% at 160 rounds), followed by ATMA (65.78\%). Krum shows limited effectiveness (43.41\%), while undefended FedAvg collapses to random guess (10\%). Data from single seed=42; multi-seed confidence intervals are provided in Table~\ref{tab:multiseed_ci}.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{visualizations/h2_roc_curve.png}
...
\begin{itemize}
    ...
    \item \textbf{Scalability Validation:} Testing with 1,000 clients confirms 99\% cost reduction at scale (L2: \$9,000 vs L1: \$900,000), with detection F1 score of 0.68 (Precision 100\%, Recall 51.5\%). L2 blockchain mechanisms~[42] provide economically viable Byzantine detection for large federated learning deployments.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{visualizations/fl_convergence.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{visualizations/atma_threshold_evolution.png}
...
Modern Byzantine adversaries can exploit on-chain transparency to refine their attack strategies~[10]. We implemented FLARE-inspired adaptive attackers that learn from blockchain logs to test the \emph{Transparency Paradox}: Does blockchain transparency help attackers more than defenders?
...
\begin{enumerate}
    ...
    \item \textbf{Limited Model Degradation:} Successful attacks degrade model accuracy by only 1.8\% on average (from 85.1\% to 83.3\%), compared to 15-20\% degradation reported for undefended systems~[35].
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{visualizations/transparency_tradeoff.png}
...
\textbf{Theoretical Foundation:} Our empirical results align with convergence guarantees established in literature. Yin et al.~[4] prove that TrimmedMean achieves convergence rate $O(\sigma^2/n + \zeta^2)$ under $f < n/2$ Byzantine clients, where $\sigma^2$ is gradient variance and $\zeta^2$ is the Byzantine attack magnitude. Blanchard et al.~[3] provide $(\alpha, f)$-Byzantine resilience bounds showing that with $f$ Byzantine clients among $n$ total, robust aggregators can approximate the true gradient within $O(f/n)$ error. Our 93.45\% accuracy with 20\% Byzantine ratio empirically validates these theoretical predictions.
...
\textbf{Hyperparameter Note:} The CIFAR-10 experiments use different hyperparameters than MNIST for dataset-specific optimization: MNIST uses \texttt{epochs=3, batch\_size=256} (optimized for simple digit classification), while CIFAR-10 uses \texttt{epochs=5, batch\_size=32} (standard for complex image classification~[1]). The multi-seed experiments (Table~\ref{tab:multiseed_ci}) use reduced settings (\texttt{epochs=3, batch\_size=64}) for computational efficiency in 3-seed validation. These differences reflect standard practice in federated learning literature where hyperparameters are tuned per-dataset.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Attack types limited to random noise and label-flip attacks. More sophisticated attack strategies, such as the ``A Little Is Enough'' (ALIE) approach proposed by Baruch et al.~[10] and backdoor attacks studied by Sun et al.~[11], were not fully evaluated in this work and require future study.
...
\begin{enumerate}
    ...
    \item \textbf{Privacy-Preserving Aggregation:} Integrate differential privacy or secure multi-party computation [22] with blockchain-verifiable proofs to balance transparency with gradient privacy.
    ...
    \item \textbf{Production Deployment:} Transition from simulated Layer-2 to live networks (Polygon, Arbitrum, Optimism) to measure real latency, throughput, and cost under production workloads [42].
...
This paper presents a comprehensive empirical study of Byzantine-robust federated learning integrated with blockchain technology, validated on both MNIST and CIFAR-10 datasets under realistic non-IID conditions. We conducted over 36 controlled experiments across multiple aggregation algorithms, attack strategies, blockchain topologies, and training durations to provide actionable insights for deploying secure federated learning systems.

\subsection{Summary of Key Findings}

\textbf{(1) Byzantine-robust aggregation does not sacrifice accuracy.} Contrary to the common assumption that robust aggregation methods trade accuracy for security, our experiments demonstrate that TrimmedMean with 160 training rounds achieves 93.45\% test accuracy on MNIST---surpassing the undefended FedAvg baseline (87.60\%) by 5.85\%. This is possible because TrimmedMean's coordinate-wise trimming filters not only Byzantine updates but also noisy gradients from heterogeneous non-IID data. On the more challenging CIFAR-10 with Dirichlet($\alpha$=0.5) distribution, TrimmedMean achieves 67.92\% under aggressive label-flip attacks (scale=-5.0), while FedAvg collapses to random guess (10\%).

\textbf{(2) Adaptive aggregation provides measurable benefit.} ATMA, which dynamically adjusts its trimming threshold based on gradient variance and kurtosis, achieves 85.12\% on MNIST (50 rounds) and 65.78\% on CIFAR-10 (160 rounds). The +0.73\% advantage over static TrimmedMean in blockchain environments, confirmed by paired t-test ($p=0.002$, Cohen's $d=0.57$), demonstrates that adaptive defense provides statistically significant benefit without manual threshold tuning.

\textbf{(3) General federated optimization methods are not Byzantine-robust.} FedProx ($\mu$=0.01) and FedDyn ($\alpha$=0.01), designed for non-IID data heterogeneity, both collapse entirely (10\% accuracy) under Byzantine attacks on CIFAR-10. This confirms that methods addressing statistical heterogeneity do not inherently provide Byzantine resilience---specialized robust aggregation is essential.

\textbf{(4) Blockchain transparency benefits defenders more than attackers.} Our Transparency Paradox experiments show that blockchain-informed FLARE-style adaptive attackers achieve only 11.6\% success rate with merely 1.8\% model degradation. In contrast, defenders gain: (a)~100\% forensic coverage (all 58 attack attempts permanently recorded), (b)~94.3\% real-time detection accuracy, and (c)~reputation-based defense capability that reduces repeat attack success to 2.3\%.

\subsection{Comparative Analysis with Prior Work}

Table~\ref{tab:contribution_comparison} positions our work against key prior studies.

\begin{table*}[t]
\caption{Comparative Analysis with Prior Work}
\label{tab:contribution_comparison}
\centering
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{Study} & \textbf{Robust Agg.} & \textbf{Blockchain} & \textbf{Adaptive} & \textbf{Non-IID} & \textbf{Transparency} & \textbf{Multi-Seed} \\
& & \textbf{Audit} & \textbf{Defense} & \textbf{Validation} & \textbf{Paradox} & \textbf{CI} \\
\midrule
Blanchard et al.~[3] (Krum) & \checkmark & & & & & \\
Yin et al.~[4] (TrimmedMean) & \checkmark & & & & & \\
Shayan et al.~[27] (Biscotti) & \checkmark & \checkmark & & & & \\
Kalibbala et al.~[46] (ATMA) & \checkmark & & \checkmark & \checkmark & & \\
Cao et al.~[35] (Survey 2024) & \checkmark & & & \checkmark & & \\
\textbf{This Work} & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table*}

Our primary contributions beyond prior work are: (a)~integrating blockchain auditability with Byzantine-robust aggregation, which no prior study has empirically evaluated end-to-end; (b)~introducing and resolving the Transparency Paradox through controlled experiments with adaptive attackers; (c)~validating ATMA in blockchain environments, demonstrating that adaptive aggregation maintains consistency across centralized and decentralized architectures (only 0.16\% accuracy difference); and (d)~providing multi-seed confidence intervals and cross-dataset validation (MNIST + CIFAR-10) for statistical rigor.

\subsection{Practical Contributions and Deployment Guidelines}

Based on our empirical findings, we provide concrete deployment recommendations:

\begin{itemize}
    \item \textbf{Algorithm selection:} Use TrimmedMean ($\beta=0.2$) for applications requiring peak accuracy with stable threat models and sufficient training budget (160+ rounds). Use ATMA for dynamic environments with unknown or variable Byzantine ratios, where automatic threshold adaptation eliminates manual tuning.
    
    \item \textbf{Cost management:} Ethereum Layer-1 deployment costs \$48,391 for 160 rounds (at 50~Gwei, \$3,000~ETH). Layer-2 solutions reduce this by 99\% to approximately \$484, making blockchain-integrated FL economically viable for production deployment.
    
    \item \textbf{Scalability:} Our 1,000-client scalability tests confirm that the system maintains Byzantine detection (F1=0.68) at scale, with L2 blockchain costs of \$9,000 (vs.~\$900,000 on L1).
    
    \item \textbf{Security posture:} Implement multi-layered defense combining robust aggregation (TrimmedMean/ATMA) + blockchain-based detection + reputation systems. This defense-in-depth approach limits even blockchain-informed adaptive attackers to 1.8\% model degradation.
\end{itemize}

\subsection{Limitations}

We acknowledge the following limitations. (1)~Attack types are limited to label-flip, sign-flip, and random noise; more sophisticated attacks such as the ``A Little Is Enough'' (ALIE) approach [10] and backdoor attacks [11] require future study. (2)~All blockchain experiments use Ganache local simulation, which provides accurate gas measurements but does not capture real-world network latency (12--15s block times) or gas price volatility. (3)~Krum's poor performance (26.56\% on MNIST, 43.41\% on CIFAR-10) was observed only under our aggressive attack setting; clean-baseline Krum experiments were not conducted. (4)~The ATMA evaluation is limited to 50 rounds for most configurations; 160-round ATMA experiments are available for CIFAR-10 only.

\subsection{Future Work}

Key directions for future research include: (1)~testnet deployment on Sepolia/Arbitrum to validate blockchain costs under real network conditions, (2)~extended ATMA evaluation with 160-round training on MNIST to determine whether adaptive methods can match TrimmedMean's 93.45\% peak, (3)~hybrid adaptive-static aggregation combining ATMA's dynamic threshold with TrimmedMean's long-term convergence, (4)~advanced adaptive attack evaluation including ALIE and backdoor strategies, (5)~cross-organizational federated learning using shared blockchain for trustless multi-party coordination, and (6)~production deployment on live Layer-2 networks (Polygon, Arbitrum, Optimism).
