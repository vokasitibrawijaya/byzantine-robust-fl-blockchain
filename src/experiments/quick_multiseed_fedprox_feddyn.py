"""
Quick Multi-Seed Experiment with FedProx and FedDyn
Optimized version for faster validation

Addresses:
- ModW2 (Confidence Intervals) with 3 seeds
- ModW3 (Recent Methods) with FedProx and FedDyn
...
# Experiment Configuration (OPTIMIZED FOR SPEED)
NUM_CLIENTS = 20
BYZANTINE_RATIO = 0.2
...
LOCAL_EPOCHS = 3  # Reduced for speed
NUM_ROUNDS = 50   # Reduced for speed (was 160)

# Multi-seed configuration (reduced to 3 for speed)
SEEDS = [42, 123, 456]
    ...
    for epoch in range(epochs):
        for images, labels in data_loader:
            ...
            for name, param in model.named_parameters():
                ...
                global_flat = global_params[name].view(-1).detach()  # Ensure detached
                h_i_slice = h_i[idx:idx + len(param_flat)].detach()  # Ensure detached
                
                # - h_i^T * (w - w_global)
                diff = param_flat - global_flat
                loss = loss - torch.dot(h_i_slice, diff)
                
                # + (alpha/2) * ||w - w_global||^2
                loss = loss + (alpha / 2) * (diff ** 2).sum()
...
def generate_summary_report(results, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  - FedProx μ: {FEDPROX_MU}\n")
        f.write(f"  - FedDyn α: {FEDDYN_ALPHA}\n\n")
        ...
        f.write(f"  - Multiple seeds: {len(SEEDS)} runs per method\n")
...
if __name__ == "__main__":
    print("="*80)
    print("QUICK MULTI-SEED EXPERIMENT")
    ...
    print(f"Seeds: {SEEDS}")
    print(f"Rounds: {NUM_ROUNDS} (optimized)")
    print(f"Total experiments: {6 * len(SEEDS)} runs")
    print(f"Estimated time: ~{6 * len(SEEDS) * 15} minutes ({(6 * len(SEEDS) * 15) / 60:.1f} hours)")
